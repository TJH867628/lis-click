        ...
        <main class="table">
            ...
            <section class="table__body">
                <table>
                    ...
                    <tbody>
                        @if($conferencesDownload->isNotEmpty())
                        @php
                            $count = 0;
                        @endphp
                            @foreach($conferencesDownload as $thisConferencesDownload)
                            <form method="post" action="{{ route('editExistingConferencesDownload',['id' => $thisConferencesDownload->field_id]) }}" enctype="multipart/form-data">
                            @csrf
                            @php
                                $count++
                            @endphp
                            <tr>
                                ...
                                <td id="titleTableData">
                                    <textarea class="form-control"  type="text" id="existingTitle" name="title">{{ trim($thisConferencesDownload->field_details) }}</textarea>
                                </td>
                                <td>
                                    <a href="{{ route('conferencesDownload', ['filename' => $thisConferencesDownload->field_value ]) }}" target="_blank">
                                        <div id="downloadButton" class="d-flex justify-content-center align-items-center">
                                            <i class="material-icons">open_in_new</i> Download Current File
                                        </div>
                                    </a>
                                ...
                                <td style="text-align: center;">
                                    <label class="switch">
                                        <input type="checkbox" name="visibility" style="display: none;" {{ $thisConferencesDownload->field_visibility == 1 ? 'checked' : null }} > <!-- Hide the checkbox -->
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>
                                    {{ $thisConferencesDownload->updated_at }}
                                </td>
                        ...
                        @if($count == 0)
                        <tr>
                            <td colspan="6">The Conferences Download List Is Empty</td>
                        </tr>
                        @endif
                        <tr>
                ...
                <div id="popup">
                    <div id="popup-content">
                        ...
                        <form method="post" action="{{ route('uploadNewConferencesDownload') }}" enctype="multipart/form-data">
                            @csrf
                            <table border>
                                <tr>
