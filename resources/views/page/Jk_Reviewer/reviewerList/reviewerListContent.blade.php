                      ...
                      <tbody>
                        ...
                        <tr>
                            ...
                            @if($reviewer->status === 0)
                              <td>Deactived</td>
                            @elseif($reviewer->status === 1)
                              <td>Active</td>
                            @endif
                            <td>{{ $reviewer->reviewerRole }}</td>
                            <td>{{ $reviewer->IC_No }}</td>
                            <td>{{ $reviewer->organizationName }}</td>
                            <td>{{ $reviewer->created_at }}</td>
                            <td>{{ $reviewer->updated_at }}</td>
                            @if($reviewer->status === 0)
                            <td><a href="{{ route('activeAdmin', ['adminEmail' => $reviewer->email]) }}" class="btn btn-primary mb-4">Active Reviewer</a></td>
                            @elseif($reviewer->status === 1)
                                <td><a href="{{ route('deactiveAdmin', ['adminEmail' => $reviewer->email]) }}" class="btn btn-primary mb-4">Deactive Reviewer</a></td>
                            @endif
                        </tr>
                        @endforeach
                      </tbody>
